...
export default class LayersManager extends React.Component<LayersManagerPropsT, LayersManagerStateT> {
  constructor(props: LayersManagerPropsT) {
    ...
    _defineProperty(this, "onDocumentClick", (event: MouseEvent) => {
      const docClickHandler = this.state.docClickHandlers[this.state.docClickHandlers.length - 1];
      if (docClickHandler) {
        docClickHandler(event);
      }
    });
    _defineProperty(this, "onKeyUp", (event: KeyboardEvent) => {
      if (event.key === 'Escape') {
    ...
    _defineProperty(this, "onAddEscapeHandler", (escapeKeyHandler: (() => mixed)) => {
      this.setState(prev => {
        return {
    ...
    _defineProperty(this, "onRemoveEscapeHandler", (escapeKeyHandler: (() => mixed)) => {
      this.setState(prev => {
        return {
    ...
    _defineProperty(this, "onAddDocClickHandler", (docClickHandler: ((event: MouseEvent) => mixed)) => {
      this.setState(prev => {
        return {
    ...
    _defineProperty(this, "onRemoveDocClickHandler", (docClickHandler: ((event: MouseEvent) => mixed)) => {
      this.setState(prev => {
        return {
