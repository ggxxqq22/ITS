...
function DatetimeFilter(props) {
  ...
  const [categoricalOperator, setCategoricalOperator] = React.useState<ValueT>([initialState.categoricalOperator]); // eslint-disable-next-line flowtype/no-weak-types
  const [rangeDates, setRangeDates] = React.useState<any>(initialState.rangeDates.length ? initialState.rangeDates : [new Date(datesSorted[0]), new Date(datesSorted[datesSorted.length - 1])]);
