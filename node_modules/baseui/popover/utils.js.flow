...
export function fromPopperPlacement(placement: string): PopoverPlacementT | null {
  const popoverPlacement: string = placement.replace(/(top|bottom)-start$/, '$1Left').replace(/(top|bottom)-end$/, '$1Right').replace(/(left|right)-start$/, '$1Top').replace(/(left|right)-end$/, '$1Bottom');
...
export function splitPlacement(placement: PopoverPlacementT): string[] {
  ...
  return (matches: string[]).slice(1, 3).filter(Boolean).map(s => s.toLowerCase());
}
/**
 ...
 */
export function getPopoverMarginStyles(showArrow: boolean, placement: PopoverPlacementT) {
 ...
 */
export function getStartPosition(offset: OffsetT, placement: PopoverPlacementT, showArrow: boolean) {
  offset = { ...offset
  };
  const [position] = splitPlacement(placement);
