...
export const getParentId = (nodes: TreeNodeT[], nodeId: TreeNodeIdT, parentId: TreeNodeIdT | null, getId: ((TreeNodeT) => TreeNodeIdT)) => {
...
export const getPrevId = (nodes: TreeNodeT[], nodeId: TreeNodeIdT, parentId: TreeNodeIdT | null, getId: ((TreeNodeT) => TreeNodeIdT)) => {
  for (let i = 0; i < nodes.length; i++) {
    if (getId(nodes[i]) === nodeId) {
      if (i === 0) {
        return parentId;
...
export const getFirstChildId = (nodes: TreeNodeT[], nodeId: TreeNodeIdT, getId: ((TreeNodeT) => TreeNodeIdT)) => {
  for (let i = 0; i < nodes.length; i++) {
    if (getId(nodes[i]) === nodeId) {
      if (nodes[i].isExpanded && nodes[i].children && nodes[i].children.length) {
...
export const getNextId = (nodes: TreeNodeT[], nodeId: TreeNodeIdT, closestOmmer: TreeNodeIdT | null, getId: ((TreeNodeT) => TreeNodeIdT)) => {
  for (let i = 0; i < nodes.length; i++) {
    if (getId(nodes[i]) === nodeId) {
      if (nodes[i].isExpanded && nodes[i].children && nodes[i].children.length) {
        return getId(nodes[i].children[0]);
...
export const getExpandableSiblings = (nodes: TreeNodeT[], nodeId: TreeNodeIdT, getId: ((TreeNodeT) => TreeNodeIdT)) => {
  for (let i = 0; i < nodes.length; i++) {
    if (getId(nodes[i]) === nodeId) {
...
export const toggleIsExpanded = (arr: TreeNodeT[], toggledNode: TreeNodeT, getId?: (node: TreeNodeT) => TreeNodeIdT = (node: TreeNodeT) => node.id ? node.id : ''): TreeNodeT[] => {
